...
contract TimelockVaultV2 {
  ...
  //=== Internal Functions
  function _depositToToken(uint _totalDeposit) internal pure returns (uint _tokenAmount) {
  ...
  function deposit(uint256 _unlockTime) external payable returns (uint256) {
    ...
    uint tokenWorth = _depositToToken(msg.value);
    require(token.transfer(msg.sender, tokenWorth), "Token transfer failed");

    // Mint tokens to user
    token.mint(msg.sender, tokenWorth);

    // Create new vault
    vaults[msg.sender].push(
      Vault({balance: msg.value, unlockTime: _unlockTime, tokenBalance: tokenWorth, active: true})
    );
  ...
  function withdrawAll() external returns (uint256) {
    ...
    for (uint256 i = 0; i < userVaults.length; i++) {
      if (
        ...
        totalWithdrawn += userVaults[i].balance;
        totalTokens += userVaults[i].tokenBalance;

        userVaults[i].balance = 0;
        userVaults[i].tokenBalance = 0;
        userVaults[i].active = false;

        emit Withdrawn(msg.sender, i, userVaults[i].balance);
      }
    }
